...
def personalLine(label, mark):
    fig, ax = plt.subplots()
    ax.fmt_ydata = 1.0
    plt.plot(tmpdf['epoc'], tmpdf[label], mark)
    # plt.plot(tmpdf['epoc'], tmpdf[label], label=label)
    plt.xlabel('epoc')
    plt.ylabel('missmatch/missmatch + match')
    plt.title("LP Missmatch")
    plt.legend()
    plt.savefig('pravesh'+'.png')
...
plt.plot(tmpdf['epoc'], tmpdf['l1d'], 'bo')  # Plot some data on the (implicit) axes.
plt.plot(tmpdf['epoc'], tmpdf['l2'], 'r^')  # etc.
plt.plot(tmpdf['epoc'], tmpdf['l3'], 'g+')
plt.xlabel('epoc')
plt.ylabel('missmatch/missmatch + match')
plt.title("LP Missmatch")
plt.legend()
